...
每日自动聚合 AI 前沿资讯的静态网页。RSS 抓取 → 去重排序 → 翻译 → 生成 JSON → 静态页面渲染，历史存档可回溯，标题/摘要/界面文案支持中英双语切换（右上角按钮，选择会记在浏览器本地）。

## 关于中英双语

抓取脚本会用免费翻译后端（先 Google 翻译的公开接口，失败再退到 MyMemory）把选中的 10 条标题和摘要译成中文，原文英文一并保留在 JSON 里（`title.en` / `title.zh`）。这两个免费后端偶尔会限流或抽风——脚本对每条做了失败兜底，翻译失败就直接留英文原文，不会导致整条流水线失败，只是那一条暂时没有中文而已。如果之后想要更稳定的翻译质量，可以换成 DeepL / 有 Key 的付费 API，把 `scripts/fetch_news.py` 里 `translate_to_zh()` 换掉即可。
